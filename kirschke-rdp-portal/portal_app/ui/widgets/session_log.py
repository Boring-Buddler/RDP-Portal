...
from PySide6.QtWidgets import (
    ...
    QHeaderView, QPushButton, QComboBox, QDateEdit, QFrame, QLabel
)
...
from portal_app.models.session import SessionLog, SessionEvent, create_mock_session_events
from portal_app.models.user import User
from shared.enums import EventType, EventResult
...
class SessionLogWidget(QWidget):
    ...
    def _create_header(self) -> QFrame:
        ...
        # Export button
        export_btn = QPushButton("Exportieren (CSV)")
        export_btn.setStyleSheet(DesignSystem.styles.button_secondary())
    ...
    def _create_filter_bar(self) -> QFrame:
        ...
        self.ws_filter.setStyleSheet(DesignSystem.styles.input_field())
        ...
        self.user_filter.setStyleSheet(DesignSystem.styles.input_field())
        ...
        self.type_filter.setStyleSheet(DesignSystem.styles.input_field())
        ...
        self.date_from.setStyleSheet(DesignSystem.styles.input_field())
        ...
        self.date_to.setStyleSheet(DesignSystem.styles.input_field())
    ...
    def _populate_workstation_filter(self) -> None:
        ...
        workstation_ids = set(event.workstation_id for event in self.all_events)
        ...
        for row, event in enumerate(sorted_events):
            ...
            if event.result:
                if event.result == EventResult.SUCCESS:
                    result_cell.setTextColor(QColor(Colors.success.name()))
                elif event.result == EventResult.FAILED:
                    result_cell.setTextColor(QColor(Colors.error.name()))
                elif event.result == EventResult.WARNING:
                    result_cell.setTextColor(QColor(Colors.warning.name()))
    ...
    def _on_export(self) -> None:
        ...
        QMessageBox.information(
            self,
            "Export",
            "CSV-Export wird in einer spaeteren Version implementiert."
        )
