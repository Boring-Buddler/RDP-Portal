...
class RDPSessionLauncher:
    ...
    def __init__(self, rdp_generator: RDPFileGenerator | None = None):
    ...
    def focus_session(self, workstation_id: str) -> bool:
        """Raise the RDP window this portal started for one machine.

        Only processes the portal launched itself are considered, so this can
        never reach a window belonging to something else.
        """
        from portal_app.rdp.window_focus import focus_windows
    ...
    def rdp_client_available(self) -> bool:
        """Whether Windows provides mstsc.exe (used by the preflight report)."""
        return self._find_mstsc() is not None

    def _find_mstsc(self) -> str | None:
    ...
    def test_rdp_file(self, rdp_file: str) -> tuple[bool, str]:
        ...
        """
        try:
            with open(rdp_file, encoding="utf-8") as f:
                content = f.read()
...
def focus_rdp_session(workstation_id: str) -> bool:
    """Bring the portal's own RDP window for one machine back to the front."""
    return _launcher.focus_session(workstation_id)
...
def cleanup_old_rdp_files(older_than_hours: int = 24) -> int:
    """Remove .rdp files a previous portal run left behind.

    Called at startup: the files name the target and the user, and a crash or
    "End task" skips the cleanup in closeEvent.
    """
    return _launcher.generator.cleanup_old(older_than_hours)
